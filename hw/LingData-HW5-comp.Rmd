...
```{r 1.1}
ggplot(shva, aes(x=f2, y=f1, color=vowel)) + geom_point() + scale_x_reverse() + scale_y_reverse()
```
...
library(ggplot2)
# f1 boxplot
ggplot(shva, aes(x=vowel, y=f1, fill=vowel)) + geom_boxplot(alpha=0.3) + coord_flip()
# f2 boxplot
ggplot(shva, aes(x=vowel, y=f2, fill=vowel)) + geom_boxplot(alpha=0.3) + coord_flip()
```

### 1.3 
Calculate Pearson's correlation of `f1` and `f2` (all data)
```{r 1.3}
cor.test(x=shva$f1, y=shva$f2)
```
...
```{r 2.1.4}
ggplot(shva, aes(x=f2, y=f1)) + geom_point() + scale_x_reverse() + scale_y_reverse() + geom_smooth(method = lm)
```
...
```{r 2.1.5}
shva1 <- shva%>% mutate(vowel=factor(vowel))
model <- lm(data=shva1, f1 ~ f2 + vowel)
summary(model)
shva %>% 
  ggplot(aes(f2, f1, color=vowel))+
  geom_point()+
  facet_wrap(~shva$vowel)
```

### 2.2.1 
Use the linear regression model to predict `f2` by `f1` and `vowel`.
```{r 2.2.1}
fit1 <- lm(data = shva, f2 ~ f1 + vowel)
summary(fit1)
```
...
```
Vowels are significant predictors for f2, but f1 isn't. 
```
...
```
```
Aux and CeSynt should be analysed using Fisher's Exact Test because we got a warning that chi-squared approximation may be incorrect.
```
...
odds <- (observed[1,1]/observed[2,1])/(observed[1,2]/observed[2,2])
odds
```
...
odds <- (expected[1,1]/expected[2,1])/(expected[1,2]/expected[2,2])
odds
```
What can you say about odds ratio of expected frequencies for arbitrary data?

```
The oddw ratio is >1 so `Aux` and `EPTrans`variables are not independant.
```
...
```{r 3.5}
library(lsr)
cramersV(res$observed)
```

### 3.6. 
Report the results of independence test using the following template:
```
We have found a significant association between variables Aux and EPTrans (p < 0.001).  The odds of ... were 2.6 times higher in (group observed) than in (group expected). Effect size is small (Cramer's V = 0.1691587).
```
...
```{r 3.7}
library(vcd)
mosaic( ~ d_caus$EPTrans + d_caus$Aux, shade=TRUE, legend=TRUE)
```
...
```
Because when we test all the pairs without adjusting for the multiplicity some variables may be signofocant, but when we adjust they become insignificant (and vice versa).
```

### 3.9 
Provide a short text (300 words) describing the hypothesis of this study and the results of your analysis.
```
We wanted to test weather there are any significant correlation between variable `Aux` and other ones. The H0 was that there is no correlation between `Aux` and other parametres. We tested a hypothesys that `Aux` and `EPTrans` are not independant with the help of Pearson's Chi-squared Test and odds ratio. It turned out to be true. 
```
